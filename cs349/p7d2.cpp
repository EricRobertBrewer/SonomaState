...
Pebbles - Choosing the lightest tile algorithm
*/
...
int main(){
  ...
  for( i = 2; i < dheight-2; i++ ){
    for( j = 2; j < dheight-2; j++ )
      fout << setw(4) << right << board[i][j] << " ";
    fout << "\n\n";
  }
  fout << "\n";
  for( i = 2; i < dheight-2; i++ ){
    for( j = 2; j < dheight-2; j++ )
      fout << setw(4) << right << sunkenboard[i][j] << " ";
    fout << "      ->   ";
  ...
  do{
    ...
    // Find the first available tile - it becomes the relative light
    coord1 = -1;
    coord2 = -1;
	...
	for( j = 2; j < dheight-2; j++ )
      ...
      // For each of the lowest tile's surrounding tiles' surrounding tiles,
      //  soften their value; we don't want to take away a tile's value twice
      //  from the grand total. Also, make the lightest tile's surrounding
      //  tiles unavailable.
