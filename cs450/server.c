...
int main(int argc, char **argv)
{
    ...
    // Bind the server end-point using the specifications stored in "servaddr"
    if( bind(listenfd, (struct sockaddr *) &servaddr, sizeof(servaddr)) < 0 ) {
	fprintf( stderr, "Bind failed.  %s\n", strerror( errno ) );
	exit( 1 );
    }

    // Listen on the in-coming connections; pile up at most LISTENQ number of connections.   
